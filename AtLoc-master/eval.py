...
for idx, (data, target) in enumerate(loader):
    ...
    # if opt.dataset == 'comballaz':
    #     #print(idx,": Output:", output)
    #     #print(idx,": Target", target)
    #     distances.append(math.sqrt(math.pow(target[0][0]-output[0][0],2)+math.pow(target[0][1]-output[0][1],2)+math.pow(target[0][2]-output[0][2],2)))
    ...
    if opt.dataset == 'comballaz' or opt.dataset == 'EPFL':
        #print(idx,": Output:", output)
        #print(idx,": Target", target)
        dist = math.sqrt(math.pow(targ_poses[idx][0]-pred_poses[idx][0],2)+math.pow(targ_poses[idx][1]-pred_poses[idx][1],2)+math.pow(targ_poses[idx][2]-pred_poses[idx][2],2))

        distances.append(dist)
        if dist < distances[minDist]:
            minDist = idx
        if dist > distances[maxDist]:
            maxDist = idx
        print(idx,": ",distances[idx])
...
if opt.dataset == 'comballaz' or opt.dataset == 'EPFL':
    print(
        'Including height: Median distance error {:3.2f} m,  mean distance error {:3.2f} m' \
        .format(np.median(distances), np.mean(distances) ))
    split = int(len(distances)//2)
    print(
        'Real Data (Assuming 50/50 Split of data): Median distance error {:3.2f} m,  mean distance error {:3.2f} m' \
            .format(np.median(distances[:split]), np.mean(distances[:split]) ))
    print(
        'Synthetic Data: Median distance error {:3.2f} m,  mean distance error {:3.2f} m' \
            .format(np.median(distances[split:]), np.mean(distances[split:])))
    print(
        'Maximum Distance {:3.2f} m,  at index {:3.2f}' \
            .format(distances[maxDist],maxDist))
    print(
        'Minimum Distance {:3.2f} m,  at index {:3.2f}' \
            .format(distances[minDist],minDist))
fig = plt.figure()
